...
myPlot2 <- myPlot1 + theme(
  axis.text.x = element_text(angle = -45, hjust = 0, vjust = 1),
  axis.text.y = element_text(angle = 45, hjust = 0, vjust = 2),
  axis.title = element_text(size = 12, face="bold", colour="maroon"),
  plot.title = element_text(size = 15, hjust = 0.5, face = "bold", colour="maroon"),
  plot.subtitle = element_text(size = 10, hjust = 0.5, face = "italic", colour = "black")
)

# Plot it
myPlot2

# change the line and point colors
myPlot3 <- myPlot2 + geom_line(aes(colour = "red")) + geom_point(aes(colour="blue"))
myPlot3
